...
//步进电机相关定义
uchar code FFW[4] = {0X20, 0X10, 0X08, 0X04};
uchar code REV[4] = {0X20, 0X04, 0X08, 0X10};
...
void settings(void)
{
    ...
    switch (set_num)
    {
        case 1: washtime = key_number; DisplayListChar(5, 2, send_char); break;
        case 2: drytime = key_number; DisplayListChar(5, 3, send_char); break;
        case 3: waterlevel_set = key_number; DisplayListChar(5, 4, send_char); break;
    }
    if (set_num >= 3 && keycode == 0X7E)
    {
        state = 5;
        set_num = 0;
        write_byte(drytime_save_addr, drytime_save_addr);
    }
}
...
void water_judge(void)
{
    ...
    if ((waterlevel_current[0] >= 2 && waterlevel_set == 1) || (waterlevel_current[0] >=4 && waterlevel_set == 2))
    {
        DisplayListChar(3, 1, "待启");
        if (keycode == 0XBD)
           state = 6;
    }
    else DisplayListChar(3, 1, "进水");
}
...
void main()
{
    while(1)
    {
        ...
        case 6:     //洗衣和脱水
            ...
            {
                LCDClear();
                DisplayListChar(3, 2, "紧急停机");
                DisplayListChar(5, 3, "需按键重启");
            }
            else if (state == 7)
            {
                delayms(2000);
